...
/**
 * REST controller for managing {@link test.contract.domain.InsuranceType}.
 */
@RestController
@RequestMapping("/api")
public class InsuranceTypeResource {
    ...
    public InsuranceTypeResource(InsuranceTypeRepository insuranceTypeRepository) {
    ...
    public ResponseEntity<InsuranceType> updateInsuranceType(@RequestBody InsuranceType insuranceType) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, insuranceType.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<InsuranceType> getInsuranceType(@PathVariable Long id) {
        ...
        Optional<InsuranceType> insuranceType = insuranceTypeRepository.findById(id);
