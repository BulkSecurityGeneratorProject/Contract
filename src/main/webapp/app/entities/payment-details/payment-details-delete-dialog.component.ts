...
export class PaymentDetailsDeletePopupComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.activatedRoute.data.subscribe(({ paymentDetails }) => {
      setTimeout(() => {
        this.ngbModalRef = this.modalService.open(PaymentDetailsDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
        this.ngbModalRef.componentInstance.paymentDetails = paymentDetails;
        this.ngbModalRef.result.then(
          result => {
            this.router.navigate(['/payment-details', { outlets: { popup: null } }]);
